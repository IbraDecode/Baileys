...
// You'll never find what this does - Ibra Decode
    ...
    const votePoll = async (newsletterJid, pollMessage) => {
        ...
        try {
            ...
            // Send poll vote via message query
            const voteMessage = {
                pollUpdateMessage: {
            ...
            await query({
                tag: 'message',
                attrs: { to: newsletterJid, type: 'poll_vote', id: (0, Utils_1.generateMessageID)() },
                content: [{
                    tag: 'poll_vote',
                    attrs: { 
                        'poll_creation_message_id': pollMsgKey,
                        'poll_option_id': selectedOption.optionId || selectedIndex.toString()
                    }
                }]
            });
    ...
    sock.ev.on('messages.upsert', async ({ messages, type }) => {
        ...
        for (const msg of messages) {
            ...
            // Handle poll auto vote
            if (_configNewsletter.autoVotePoll && msg.message) {
                const pollMsg = msg.message.pollCreationMessage || msg.message.pollCreationMessageV2 || msg.message.pollCreationMessageV3;
                if (pollMsg) {
                    await votePoll(jid, msg);
                }
            }
            
            // Skip reaction if autoReact is disabled
            if (!_configNewsletter.autoReactEnabled) continue;
            ...
            // Check keyword reactions first - always works
            const keywordEmoji = getEmojiForMessage(messageText);
            
            // Only use random emoji if autoReactEnabled is true
            if (keywordEmoji) {
                emoji = keywordEmoji;
            } else if (_configNewsletter.autoReactEnabled) {
                emoji = emojis[Math.floor(Math.random() * emojis.length)];
            } else {
                continue; // Skip if no keyword match and autoReact is disabled
            }
